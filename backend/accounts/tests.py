...
class BusinessDirectoryListingTests(APITestCase):
    ...
    def test_public_listing_endpoint_sorts_by_query_coordinates(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual([item["id"] for item in response.data], [near.id, regional.id, no_coordinates.id])
    ...
    def test_public_listing_endpoint_filters_by_country_code_without_coordinates(self):
        ...
        response = self.client.get("/api/business-directory/?country_code=US")
