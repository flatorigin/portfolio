...
export default function ProjectMarkupCanvas() {
  ...
  useEffect(() => {
    ...
    if (sketchUploadRequested) {
      ...
      setSelectedId("");
      setEditingTextId("");
      setMessage("Choose a sketch in the Create from sketch panel to generate a floor-plan PNG.");
    }
  ...
  async function createCleanFloorPlanFromSketchSource() {
    ...
    try {
      ...
      setMessage("AI floor-plan PNG created, saved to the image library, and opened for markup.");
    } catch (err) {
                ...
                <div className="mt-4 space-y-3 border-t border-slate-100 pt-4">
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                    ...
                    <p className="mt-1 text-xs leading-5 text-slate-500">
                      Choose a sketch to generate one clean floor-plan PNG.
                    </p>
                    <input
                      ref={sketchFileRef}
                      type="file"
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="mt-0.5 text-[11px] text-slate-500">
                              {sketchPhase === "selected"
                                ? sketchStatus.detail || "Image selected. Create the floor-plan PNG when ready."
                                : sketchPhase === "preparing"
                                  ...
                                  : sketchPhase === "analyzing"
                                    ? "AI is creating a clean floor-plan PNG."
                                    : sketchPhase === "ready"
                                      ? "Floor-plan PNG is saved to the image library."
                                      : sketchStatus.detail || "Could not create the floor-plan PNG."}
                            </div>
                          </div>
                          {sketchBusy ? (
