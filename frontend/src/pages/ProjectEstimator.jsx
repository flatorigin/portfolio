...
function EstimatorEntry() {
  ...
  const categoryName = tradeTitle || (isElectrical ? 'Electrical' : isDecking ? 'Decking' : isSiding ? 'Siding' : isFlooring ? 'Flooring' : isRoofing ? 'Roofing' : isPaving ? 'Paving' : isDrywall ? 'Drywall' : isFraming ? 'Framing' : 'Painting');
  return (
    ...
    const load = async () => {
      ...
      try {
        if (isNew) {
          ...
          if (['framing', 'drywall', 'paving', 'roofing', 'flooring', 'siding', 'decking', 'fencing', 'windows', 'doors', 'garage_coating', 'electrical'].includes(data.estimate_type)) {
