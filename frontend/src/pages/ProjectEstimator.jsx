...
function EstimatorEntry() {
  ...
  const categoryName = isDrywall ? 'Drywall' : isFraming ? 'Framing' : 'Painting';
  return (
    <div className="min-h-screen bg-[#FBF9F7] py-12 sm:py-20">
      <Container>
        <div className="mx-auto max-w-3xl rounded-xl border border-slate-200 bg-white px-6 py-12 text-center shadow-sm sm:px-12">
          ...
          <label className="mx-auto mt-7 block max-w-sm text-left"><FieldLabel>Estimator type</FieldLabel><Select value={estimatorType} onChange={(event) => setEstimatorType(event.target.value)}><option value="painting">Painting</option><option value="framing">Framing</option><option value="drywall">Drywall</option></Select></label>
          <div className="mx-auto mt-4 max-w-sm rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-left"><div className="text-sm font-bold text-slate-950">{categoryName} estimator</div><p className="mt-1 text-xs leading-5 text-slate-600">{isDrywall ? 'Estimate board quantities, hanging, finishing, removal, and patch repairs.' : isFraming ? "Estimate walls, openings, floors, beams, posts, roofs, hardware, labor, overhead, and profit." : "Group rooms by condition, calculate walls and ceilings, add trim work, and present detailed or summary pricing."}</p></div>
          <Link to={`/${isDrywall ? 'drywall' : isFraming ? 'framing' : 'project'}-estimator/new`} className="mt-5 inline-flex min-h-12 items-center gap-2 rounded-xl bg-slate-950 px-6 text-sm font-semibold text-white hover:bg-slate-800"><SymbolIcon name="arrow_forward" className="text-[20px]" />Start {categoryName} Estimate</Link>
    ...
    const load = async () => {
      ...
      try {
        if (isNew) {
          ...
          if (['framing', 'drywall'].includes(data.estimate_type)) {
