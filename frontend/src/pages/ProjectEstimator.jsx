...
function EstimatorEntry() {
  ...
  const isPlumbing = estimatorType === "plumbing";
  const isElectrical = estimatorType === "electrical";
  const tradeTitles = { fencing: "Fencing", windows: "Windows", doors: "Doors", garage_coating: "Garage floor coating" };
  const tradeTitle = tradeTitles[estimatorType];
  const categoryName = tradeTitle || (isPlumbing ? 'Plumbing' : isElectrical ? 'Electrical' : isDecking ? 'Decking' : isSiding ? 'Siding' : isFlooring ? 'Flooring' : isRoofing ? 'Roofing' : isPaving ? 'Paving' : isDrywall ? 'Drywall' : isFraming ? 'Framing' : 'Painting');
  return (
    <div className="min-h-screen bg-[#FBF9F7] py-12 sm:py-20">
      <Container>
        <div className="mx-auto max-w-3xl rounded-xl border border-slate-200 bg-white px-6 py-12 text-center shadow-sm sm:px-12">
          <SymbolIcon name={isElectrical ? "electrical_services" : isFraming ? "foundation" : "format_paint"} className="text-[42px] text-slate-700" />
    ...
    const load = async () => {
      ...
      try {
        if (isNew) {
          ...
          if (['framing', 'drywall', 'paving', 'roofing', 'flooring', 'siding', 'decking', 'fencing', 'windows', 'doors', 'garage_coating', 'electrical', 'plumbing'].includes(data.estimate_type)) {
