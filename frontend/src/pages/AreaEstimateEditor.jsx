...
const estimatePath = type => `/${['framing', 'drywall', 'paving', 'roofing', 'flooring', 'siding', 'decking', 'fencing', 'windows', 'doors', 'garage_coating', 'electrical', 'plumbing'].includes(type) ? type : 'project'}-estimator`;
...
export default function AreaEstimateEditor({ category = 'drywall' }) {
  ...
  return <div className="min-h-screen bg-[#FBF9F7] pb-16 text-slate-900">
    <div className="border-b border-slate-200 bg-white"><Container className="py-7 sm:py-9"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      ...
      <div className="flex gap-2">{authed && <Link to={`${route}/new`} className="inline-flex h-11 items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 text-sm font-semibold text-slate-800 hover:bg-slate-50"><SymbolIcon name="add" className="text-[18px]" />New</Link>}<LeaveEstimateButton pendingKey={pendingKey} disabled={busy} /><Button type="button" onClick={save} disabled={busy || calculating || !!previewError} className="h-11 gap-2"><SymbolIcon name={authed ? 'save' : 'person_add'} className="text-[18px]" />{authed ? 'Save estimate' : 'Create account to save'}</Button></div>
    </div></Container></div>
