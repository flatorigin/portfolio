...
import { buildBusinessShareData, isShareableBusinessListing } from "../utils/businessShare";
...
export default function BusinessShareDialog({ listing, onClose }) {
  const [feedback, setFeedback] = useState("");
  const origin = typeof window !== "undefined" ? window.location.origin : "";
  const share = useMemo(() => buildBusinessShareData(listing, origin), [listing, origin]);
  ...
  if (!isShareableBusinessListing(listing)) return null;
      ...
      <div className="w-full max-w-md rounded-lg bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4">
          <div className="min-w-0">
            <h2 id="business-share-title" className="text-lg font-semibold text-slate-950">
              Share business
            </h2>
