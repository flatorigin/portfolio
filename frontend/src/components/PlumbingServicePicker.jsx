...
import { PLUMBING_SERVICES } from "../data/plumbingPricing2026_27";

export const quickCategories = [["all", "All"], ["fixtures", "Sinks & Faucets"], ["toilets", "Toilets"], ["bathing", "Showers & Tubs"], ["appliances", "Appliances"], ["heaters", "Water Heaters"], ["drains", "Drains"], ["supply", "Pipes & Valves"], ["pumps", "Pumps"], ["remodel", "Renovations"]];
...
export default function PlumbingServicePicker({ onAdd }) {
  const id = useId();
  ...
  const matches = useMemo(() => PLUMBING_SERVICES.filter(item => !excludedServices.has(item.id) && (category === "all" || item.categoryId === category) && `${item.title} ${item.keywords}`.toLowerCase().includes(query.toLowerCase().trim())), [query, category]);
  ...
  return <div className="space-y-3" onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget)) setOpen(false); }}>
    ...
    <div className="relative">
      ...
      }} placeholder={`Search ${PLUMBING_SERVICES.length} services (e.g. toilet, leaking faucet, water heater)...`} className="h-12 w-full rounded-xl border border-stone-300 bg-white px-3 text-sm text-stone-900 focus:outline-none focus:ring-2 focus:ring-stone-400" />
