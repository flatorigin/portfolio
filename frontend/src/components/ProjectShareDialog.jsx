...
import { buildProjectShareData, isShareablePublicProject } from "../utils/projectShare";

function ShareAction({ href, icon, label, onClick, external = false }) {
  const className =
    "flex h-12 min-w-0 items-center gap-3 rounded-lg border border-slate-200 bg-white px-3 text-left text-sm font-medium text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-900/15";
...
export default function ProjectShareDialog({ open, project, onClose }) {
  const [feedback, setFeedback] = useState("");
  const origin = typeof window !== "undefined" ? window.location.origin : "";
  const share = useMemo(() => buildProjectShareData(project, origin), [origin, project]);
  ...
  if (!open || !isShareablePublicProject(project)) return null;
  ...
  return (
    <div
      className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/45 p-3 backdrop-blur-sm sm:items-center sm:p-6"
      role="dialog"
      aria-modal="true"
      ...
      <div className="w-full max-w-md rounded-lg bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4">
          <div className="min-w-0">
            <h2 id="project-share-title" className="text-lg font-semibold text-slate-950">
              Share project
            </h2>
            <p className="mt-1 truncate text-sm text-slate-500">{share.title}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="inline-flex size-9 shrink-0 items-center justify-center rounded-full text-slate-500 transition hover:bg-slate-100 hover:text-slate-900"
        ...
        <div className="p-5">
          <div className="grid grid-cols-2 gap-2">
            ...
            {nativeShareAvailable ? (
              <ShareAction icon="ios_share" label="More" onClick={openNativeShare} />
            ) : null}
          </div>
