...
import ElectricalServicePicker from "./ElectricalServicePicker";
import ElectricalEvConfiguration from "./ElectricalEvConfiguration";
import { configureEvProject, isEvInstallation } from "../data/electricalEvScope";
import {
...
export default forwardRef(function ElectricalScopeBuilder({ estimateId, shareToken }, ref) {
  ...
  function addProject(serviceId) {
    ...
    let project = createElectricalProject(serviceId);
    if (project && isEvInstallation(serviceId)) project = configureEvProject(project);
    if (!project) return;
  ...
  function addCustomLine(projectIndex) {
    ...
    updateProject(projectIndex, "line_items", [...project.line_items, {
      id: `line-custom-${crypto.randomUUID()}`, name: "Additional electrical work", quantity: 1, unit: "allowance",
  ...
  if (loading) return <div className="rounded-xl border border-stone-200 bg-white p-8 text-sm text-stone-500">Loading the shared electrical scope…</div>;
  ...
  return <section id="scope-builder" className="scroll-mt-24 rounded-xl border border-stone-200 bg-white shadow-sm">
    <header className="border-b border-stone-200 bg-white p-5 text-stone-900 sm:p-7">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div><div className="text-xs font-semibold uppercase tracking-[0.14em] text-stone-900">Itemized project scope</div><h2 className="mt-2 text-2xl font-bold">Your electrical project scope</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-stone-600">Add services, review the included work, and adjust quantities before sharing with an electrician.</p></div>
    ...
    <div className="space-y-6 p-5 sm:p-7">
      ...
      {canEdit && <ElectricalServicePicker onAdd={addProject} />}
      ...
      {draft.inputs.projects.length ? <div className="space-y-4">{draft.inputs.projects.map((project, projectIndex) => <article key={project.id} className="overflow-hidden rounded-xl border border-stone-200">
        ...
        {isEvInstallation(project.service_id) && <ElectricalEvConfiguration project={project} disabled={!canEdit} onChange={options => { setCalculation(null); setInput("projects", draft.inputs.projects.map((item, index) => index === projectIndex ? configureEvProject(item, options) : item)); }} />}
      ...
      </article>)}</div> : <div className="rounded-xl border border-dashed border-stone-300 px-5 py-10 text-center"><SymbolIcon name="electrical_services" className="text-[34px] text-stone-400" /><h3 className="mt-3 font-bold text-stone-950">No electrical projects added</h3><p className="mt-1 text-sm text-stone-600">Choose a project above to receive its minimum complete scope.</p></div>}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {shareToken && role === "homeowner" && <Link to={`/electrical-estimator/${record.id}`} className="rounded-xl border border-stone-300 px-4 py-2 text-sm font-semibold">Open owner estimate</Link>}
      </div>
    </div>
  </section>;
});
